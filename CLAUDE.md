...
## 16. Kosilium — víceúhlová porada
**Pro netriviální rozhodnutí nikdy neodpovídej jedním hlasem.**

Když uživatel řekne „zapoj kosilium", „svolej koncilium" nebo podobně, aktivuj těchto 5 poradců, každý z jiného úhlu pohledu:

1. **Oponent** — najde nejslabší místo v mém uvažování a tlačí na něj.
2. **Myslitel od základů** — ignoruje mou formulaci a řeší skutečný problém.
3. **Rozšiřovač** — všimne si příležitosti nebo pozitiva, které jsem přehlédl.
4. **Outsider** — bez znalosti kontextu upozorní na to zjevné, co mi může unikat.
5. **Implementátor** — řekne, jaký je další konkrétní krok.

Nech poradce, ať se vzájemně vyváží, slabé argumenty vyřaď a na závěr vydej **jeden jasný finální verdikt**. Pokud si nejsi jistý, řekni „nevím" místo hádání.

---

## 17. Nasazení změn (BNL Dashboard)
**Pevný postup pro každé nasazení této appky. GitHub: https://github.com/Radek-78/bnl-dashboard**

1. `git pull` — vždy nejdřív stáhnout aktuální stav z GitHub.
2. Provést požadovanou úpravu kódu.
3. Zvýšit číslo verze (`server/00_config.js` → `CONFIG.version`/`releaseDate`, `00_VERZE.js`).
4. Doplnit záznam do changelogu (`server/changelog.js`, pole `CHANGELOG`).
5. Aktualizovat splashscreen a footer (zobrazují verzi/datum vydání).
6. `git push`.
7. `clasp push`.
8. Uživatel poté ručně provede nové nasazení v Apps Script editoru (Nasadit → Spravovat nasazení) — tím je nasazení hotové. Tento krok je vždy ruční a mimo rozsah asistenta.

Kroky 1–7 prováděj bez čekání na dodatečné schválení — tento postup je předem odsouhlasený jako standardní workflow projektu.
